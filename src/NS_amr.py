...
'''
    Scorer Network, designed as a CNN
      + MaxPooling + Softmax 
   
    Input is:   low-resolution image (64,256,4)
    Output is:  abstract representation (64,256) + scores (num_patches*num_patches,1)
...
'''
    ...
    Consists of: scorer + ranker + decoder
     
    Input is:   low-resolution image (64,256,4)
    Output is:  a list of length number_of_bins. 
                Each element of the list is another list (with all patches at that resolution).
...
'''
  ...
  def __init__(self, 
    ...
    # Setting up the decoder network
  ...
  def call(self, inputs):
    ...
    # Low-resolution input goes through the scorer,
    # which returns the abstract features of the low-resolution input and the scores of each patch
    spatial_attention , scores = self._scorer(flowvar) #scores shape [BS,NP]


    #The scores go through the ranker that outputs the target resolution of each patch (I=indices)
    I     = self._ranking(scores) #indices shape [BS*NP]

    features = tf.concat([flowvar,spatial_attention],axis=-1)

    #Convert the abstract features to patches
    ...
    #Loop that goes through each target resolution
    for i , idx in enumerate(I):
     ...
     #each patch goes through the decoder
    ...
    #output, in order:
    # P: a list, where each element of the list is a list of patches at a specific resolution
    # TP: true low-resolution patches
    # I: indices, each patch at each resolution
    # XZ: final coordinates
    return P, TP, I, XZ 
