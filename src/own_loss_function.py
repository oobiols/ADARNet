...
def mse_ux(y_true,y_pred):
 ...
 return tf.keras.backend.mean(tf.keras.backend.square(y_true[:,:,:,0]-y_pred[:,:,:,0]),axis=None)

def mse_nut(y_true,y_pred):

 return tf.keras.backend.mean(tf.keras.backend.square(y_true[:,:,:,3]-y_pred[:,:,:,3]),axis=None)
