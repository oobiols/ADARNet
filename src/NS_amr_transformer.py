...
class TransformerEncoder(keras.layers.Layer):
    def __init__(self,
        ...
        self._batch_norm_1 = tf.keras.layers.BatchNormalization()
        self._multihead_attention = tf.keras.layers.MultiHeadAttention(num_heads = num_heads,value_dim = query_dim, key_dim = key_dim)
        self._batch_norm_2 = tf.keras.layers.BatchNormalization()
        self._mlp = tf.keras.layers.Dense(proj_dim,activation = tf.nn.leaky_relu)

    def call(self, inputs):

        x = self._batch_norm_1(inputs)
        x , scores = self._multihead_attention(x,x,return_attention_scores = True)
        add_1 = tf.keras.layers.Add()([x,inputs])
        x = self._batch_norm_2(add_1)
        x = self._mlp(x)
...
class PatchEmbedding(tf.keras.layers.Layer):
    def __init__(self, num_patch, proj_dim, **kwargs):
        super(PatchEmbedding, self).__init__(**kwargs)
        self.num_patch = num_patch
        #self.proj = tf.keras.layers.Dense(proj_dim)
        self.pos_embed = tf.keras.layers.Embedding(input_dim=num_patch, output_dim=proj_dim)
...
class ScorerNetwork(keras.layers.Layer):
    def __init__(self,
        ...
        self._embedding = PatchEmbedding(num_patches,proj_dim)
        for i in range(self._n_encoders):
          self._encoders.append(TransformerEncoder(num_heads,proj_dim,query_dim, key_dim))
...
class NSAmrScorer(NSModelPinn):
  def __init__(self, 
    ...
    self._enc_dec = []
    factor = 2
    self._proj_dim = self._rows_patch*self._columns_patch//(factor*factor)

    self._scorer = ScorerNetwork(num_patches=self._n_patches,proj_dim=self._proj_dim,n_encoders=6,query_dim=32,key_dim=32,num_heads=1)
    ...
    self._patches_to_image = tf.keras.layers.Reshape(target_shape=[self._rows_image//factor,self._columns_image//factor,1])
    self._encoded_patches = tf.keras.layers.Conv2D(filters = self._proj_dim,kernel_size=3,strides=(self._rows_patch,self._columns_patch),activation=tf.nn.leaky_relu)
    self._feature_upsampling = tf.keras.layers.UpSampling2D(size=(factor,factor),data_format="channels_last", interpolation='nearest')
  ...
  def call(self, inputs):
    ...
    features = self._encoded_patches(flowvar) 
    features = tf.keras.layers.Reshape(target_shape=[self._n_patches,self._proj_dim])(features)
 
    features , scores = self._scorer(features)

    features = self._patches_to_image(features)
  ...
  def test_step(self, data):
    ...
    beta_cont = float(data_loss/momyMse)

    loss = data_loss + self.beta[0]*(beta_cont*contMse + beta_momx*momxMse + beta_momy*momy_loss)
