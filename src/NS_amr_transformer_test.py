...
class ConvolutionModule(keras.layers.Layer):
    def __init__(self,
                  filters=[8,16,32,64],
                  kernel_size = 3,
                  strides = [1,1,4,4],
    ...
    def call(self,inputs):
     ...
     for layer in self._conv:
      print('layer policy ', layer.dtype_policy)
...
class NSAmrScorer(NSModelPinn):
  def __init__(self, 
    ...
    self._convolve = ConvolutionModule(filters=[8,16,self._proj_dim],kernel_size = 3, strides=[1,4,4])
    #self._scorer = ScorerNetwork(num_patches=self._n_patches,proj_dim=self._proj_dim,n_encoders=4,query_dim=self._n_patches*self._n_patches,key_dim=self._n_patches*self._n_patches,num_heads=1)
    self._scorer = ScorerNetwork(num_patches=self._n_patches,proj_dim=self._proj_dim,n_encoders=4,query_dim=512,key_dim=512,num_heads=12)
    ...
    self._patches_to_image = tf.keras.layers.Reshape(target_shape=[self._rows_image,self._columns_image,4])
  ...
  def call(self, inputs):
    ...
    features = self._convolve(flowvar) 
    ...
    #features = self._feature_downsampling(features)
