...
class PostProcessAmr():
  ...
  def levels_to_png(self):
    ...
    data = np.empty([self.patchheight,self.patchwidth])
    colors={0:3,1:2.5,2:2,3:1.5}
    ...
    for i in range(self.npatches):
     for j , indices in enumerate(self.indices[1:]):
      ...
      if i in indices:
         ...
         h = patches.shape[1]
         w = patches.shape[2]
         number_split_cells += h*w


    f.write(str(number_split_cells)+'\n(\n')

    level = 0
    r_cells = []
    for idxs in self.indices[1:]:
   
     level += 2
     npatches = len(idxs)
     cont = 0 
     for _ in range(npatches):
      ncells = (level*level+1)*self.patchheight*self.patchwidth
      for _ in range(0,ncells,level*level+1):  
        id0 = str(cont)
        id1 = str(cont+1)
        id2 = str(cont+2)
        r_cells.append(cont+2)
        id3 = str(cont+3)
        r_cells.append(cont+3)
        id4 = str(cont+4)
        r_cells.append(cont+4)
        for k in range(0,level*level+1):
          if k == 1:
            f.write('-1 8('+id0+' '+id2+' '+id3+' '+id4+')\n')
     ...
     for j , indices in enumerate(self.indices):
      for _ in indices:
         level = 1 if j == 0 else 2 
         total_n_cells+=level*level*self.patchheight*self.patchwidth


     f.write('//visibleCells\n\n'+str(total_n_cells)+'\n(\n')     
     ncells_b = 0
     ncells_e = (level*level+1)*self.patchheight*self.patchwidth
     for i in range(self.npatches):
      for j , indices in enumerate(self.indices):

       if i in indices:
        if j < 1:
         n_cells = self.patchheight*self.patchwidth
         for _ in range(n_cells):
             f.write('-1\n')

        else:

         for k in range(ncells_b,ncells_e,level*level+1):  
          f.write(str(k)+'\n')            

         ncells_b += (level*level+1)*self.patchheight*self.patchwidth
         ncells_e += (level*level+1)*self.patchheight*self.patchwidth
...
class PostProcess(PostProcessAmr):
  def __init__(self, 
                **kwargs):
