...
#
 ...
 def load_data(self,cases,patches=0,patch_size=[32,128]):
  ...
  X=np.empty([0,self.height,self.width,6],dtype=np.float16)
  Y=np.empty([0,self.height,self.width,6],dtype=np.float16)
 ...
 def __single_sample(self,
  ...
  if (self.is_turb) and self.add_coordinates:
  ...
  elif self.is_turb==False and self.add_coordinates:
 ...
 def vector_to_foam(self,X,Y,Z,variable_name="U"):
  ...
  for n in range(n_samples):
   ...
   directory_path='./predicted_fields/1/'
 ...
 def unmap_domain(self,var):
  ...
  firstb = var[:,:,:w].reshape((n_samples,height*w))
  secondb = var[:,:,w:2*w].reshape((n_samples,height*w))
  thirdb = var[:,:,2*w:3*w].reshape((n_samples,height*w))
  fourthb = var[:,:,3*w:4*w].reshape((n_samples,height*w))

  return np.concatenate([firstb,secondb,thirdb,fourthb],axis=1)
 ...
 def map_domain(self, arr):
  ...
  if self.grid=="ellipse" or self.grid=="airfoil":
    ...
    firstb = arr[:bs].reshape((bh,bw))
    secondb = arr[bs:2*bs].reshape((bh,bw))
    thirdb = arr[2*bs:3*bs].reshape((bh,bw))
    fourthb = arr[3*bs:].reshape((bh,bw))
    
    ret = np.append(firstb,secondb,axis=1)
    ret = np.append(ret,thirdb,axis=1)
    ret = np.append(ret,fourthb,axis=1)
...
class DatasetNoWarmup(Dataset):
  ...
  def create_dataset(self,
    ...
    while (case_number !=case_end) :
      ...
      if not os.path.exists("./" + self.dataset_type+"_data_"+str(self.dataset_name)+"_"+str(self.width)+"_"+str(self.height) + "/" + case ):
    
        print("case ", case," does not exist")
